...
| GET | `/comments?authorId=&limit=&cursor=` | аноним | комментарии одного автора, новые сверху, плоско: `{items, nextCursor}`. Лимиты и курсор как у листинга по subject, но курсор общий с `/replies` и сортировка одна. Ни одного из `subjectType` и `authorId` или оба сразу — 404 |
...
Листинг по автору — это личный кабинет и профиль читателя. В него попадает только то, что стоит:
ни своих удалённых, ни снятых модерацией автор в нём не видит — заглушка держит ответы в ветке и
ничего не говорит о том, кто её написал. Ответ автора идёт отдельной строкой, а не под своим
родителем, даже если оба попали на одну страницу; `replies` всегда пуст, а `repliesTruncated`
стоит там, где под комментарием есть ответы. Игнор зрителя применяется как в ветке: `soft` и
`gravestone` меняют `visibility`, `subtree_removal` убирает строку целиком, поэтому на профиле
полностью игнорируемого автора список пуст — плашку про игнор рисует клиент, он знает про свой
список. `viewer` в этом ответе не приходит: отсюда ничего не пишут.
