...
- **Сквозной тест логина.** Поток `editorial-ui` проверяется руками по сценарию,
  который живёт в истории веток, а не в репозитории. Playwright с поднятым
  Keycloak закрыл бы это, но заводить браузерную автоматизацию с самого сложного
  флоу — неудачный первый шаг; сейчас во фронте шесть юнит-тестов.
