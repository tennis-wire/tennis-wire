...
- **Истечение не выгружает вкладку.** По достижении 8 часов появляется баннер с
  ...
  заблокированном попапе и предупреждает о потере. Причина: текст статьи лежит в
  `localStorage`, а история AI-чата, диалог транскрипции и id несохранённой
  статьи — только в состоянии React.
...
- **Черновик в `localStorage` привязан к `sub`.** Ключ темы — нет, это настройка
  браузера, а не человека.
