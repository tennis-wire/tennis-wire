...
- **Читательские клиенты (`public-web`, `mobile`):** запрашивают `offline_access`; refresh-токен отвязан от SSO-сессии и живёт 30 дней простоя (`offlineSessionIdleTimeout`). Остальным клиентам этот scope из optional убран.
...
Обновление access-токена — на сервере, single-flight по `sub`: страница шлёт несколько запросов сразу, и без этого каждый пошёл бы за своим токеном. Выход дополнительно отзывает refresh-токен: offline-токен переживает завершение SSO-сессии по определению, и выброшенной куки мало.
...
Вход, выход, смена имени и удаление аккаунта собраны за одним значком в шапке и в кабинете на `/me`. Регистрация и Google на странице Keycloak и в интерфейсе не дублируются: `/api/auth/login` — единственный вход, и три способа начать за ним различаются только тем, что читатель выберет уже там. Удаление (`DELETE /api/users/me`) подтверждается вводом своего имени, а после 202 текущее устройство сразу выходит из аккаунта. Подтверждения входа перед удалением (`discussion-rules.md` §13.3) по-прежнему нет — сервер удаляет без него.
